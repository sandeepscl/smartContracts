...
contract TESTToken is ERC20, Ownable {
    ...
    function setTESTRewardsFee(uint256 value) external onlyOwner{
        ...
        totalFees = BUSDRewardsFee.add(buybackFee).add(marketingFee);
    }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialTESTBalance = IERC20(BUSD).balanceOf(address(this));

        swapTokensForTEST(tokens);
        uint256 newBalance = (IERC20(BUSD).balanceOf(address(this))).sub(initialTESTBalance);
